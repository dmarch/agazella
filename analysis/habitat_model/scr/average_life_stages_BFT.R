...
output_data <- paste("data/out/habitat_model/predict/species/BFT", paste("hi",tfreq,sep="-"), sep="/")
...
#---------------------------------------------------------------
# 3. Average habitat importance on a monthly basis
#---------------------------------------------------------------
...
foreach(i=1:length(dates), .packages=c("lubridate", "raster", "stringr", "dplyr", "pals")) %dopar% {
  ...
  # find netcdfs
  pat <- paste0(".*",  format(date, "%Y%m%d"),"_BFT_", "\\w{3}", "_hi.nc")
